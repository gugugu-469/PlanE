...
from trl import KTOConfig, KTOTrainer, get_peft_config, ModelConfig, TrlParser
...
def get_checkpoint(training_args: KTOConfig):
...
def kto_function(
    model_args: ModelConfig, script_args: ScriptArguments, training_args: KTOConfig
):
    #########################
    # Log parameters
    #########################
    ...
    def format_kto_sample(sample):
        ...
        return {"prompt": prompt, "completion": completion, "label": sample['label']}
    train_dataset = train_dataset.map(
        format_kto_sample, remove_columns=train_dataset.column_names
    )
    ...
    if trainer.accelerator.is_main_process:
        trainer.create_model_card({"tags": ["rl","kto", "tutorial", "philschmid"]})
...
def main():
    parser = TrlParser((ModelConfig, ScriptArguments, KTOConfig))
    model_args, script_args, training_args = parser.parse_args_and_config()
    model_args.trust_remote_code = True
    # Run the main training loop
    kto_function(model_args, script_args, training_args)
