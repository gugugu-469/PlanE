...
c_to_hr_prompt = '''You are currently a senior expert in fine-grained sentiment extraction.
Your task is to extract the possible aspect terms from the given text.
The output format of the task is: (Aspect Term) 
Given text: "{text}"
'''

c_to_tr_prompt = '''You are currently a senior expert in fine-grained sentiment extraction.
Your task is to extract the possible opinion terms from the given text.
The output format of the task is: (Opinion Term) 
Given text: "{text}"
'''

sc_to_tr_prompt = '''You are currently a senior expert in fine-grained sentiment extraction.
...
sc_to_hr_prompt = '''You are currently a senior expert in fine-grained sentiment extraction.
    ...
    for data_type, use_datas in zip(data_type_list, use_datas_list):
        for prompt_name, prompt_format in overall_prompt_list:
            ...
            if prompt_name in ori_task:
                ...
                read_file = os.path.join(now_out_dir, '{}_{}_14lap.jsonl'.format('c_to_hr', data_type))
                ...
                read_file = os.path.join(now_out_dir, '{}_{}_14lap.jsonl'.format('c_to_tr', data_type))
                with jsonlines.open(read_file, 'r') as f:
                    read_datas = [data for data in f]
                index_list = []
                tr_list = []
            ...
            with jsonlines.open(os.path.join(now_out_dir, out_name), 'w') as f:
                for data in out_datas:
                    f.write(data)
    end_time = time.time()
    print('双向拆解_model_{}_time:{}'.format(model_path, end_time-start_time))
