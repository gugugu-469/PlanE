...
c_to_ht_prompt = '''当前你是一个资深的信息提取的专家。
...
给定文本：“{text}”
'''
c_to_r_prompt = '''当前你是一个资深的关系分类专家。
你的任务是基于给定文本，从给定关系列表中确定可能的关系。给定的关系列表：{relation_list}。
任务的输出形式是（关系类型）。
给定文本：“{text}”
'''

rc_to_ht_prompt = '''当前你是一个资深的信息提取的专家。
...
htc_to_r_prompt = '''当前你是一个资深的关系分类专家。
    ...
    for data_type, use_datas in zip(data_type_list, use_datas_list):
        for prompt_name, prompt_format in overall_prompt_list:
            ...
            if prompt_name in ori_task:
                ...
                read_file = os.path.join(now_out_dir, '{}_{}_CMeIE.jsonl'.format('c_to_r', data_type))
                ...
                read_file = os.path.join(now_out_dir, '{}_{}_CMeIE.jsonl'.format('c_to_ht', data_type))
                with jsonlines.open(read_file, 'r') as f:
                    read_datas = [data for data in f]
                index_list = []
                sp_list = []
                for index,data in enumerate(read_datas):
                    ...
                    for sp in data['sp_list']:
                        text_list.append(text)
                        head_tail = '（{}||{}||{}||{}）'.format(sp['subject'], sp['subject_type'], sp['object'], sp['object_type'])
                        prompt = prompt_format.format(
                            relation_list=trip_types,
                            entity_list = ent_types,
                            text=data['text'],
